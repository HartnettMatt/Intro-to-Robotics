...
def steer(from_point, to_point, delta_q):
    ...
    '''
    # Figure out if you can use "to_point" as-is, or if you need to move it so that it's only delta_q distance away
    dist = np.linalg.norm(to_point - from_point)
    if (dist > delta_q):
        to_point = from_point + delta_q*((to_point - from_point)/dist)
    # Use the np.linspace function to get 10 points along the path from "from_point" to "to_point"
...
def rrt(state_bounds, state_is_valid, starting_point, goal_point, k, delta_q):
    ...
    # Your code here
    # Make sure to add every node you create onto node_list, and to set node.parent and node.path_from_parent for each
    # get_nearest_vertex(node_list, (0,2))
    for i in range(k):
        new_point = get_random_valid_vertex(state_is_valid, state_bounds)
        if goal_point is not None:
            if random.random() < 0.05:
                new_point = goal_point
        nearest_node = get_nearest_vertex(node_list, new_point)
        path = steer(nearest_node.point, new_point, delta_q)
        new_point = path[9]
        if(check_path_valid(path, state_is_valid)):
            new_node = Node(new_point, parent=nearest_node)
            new_node.path_from_parent = path
            node_list.append(new_node)
            if goal_point is not None and np.linalg.norm(new_point-goal_point) <= 0.00001:
                return node_list
